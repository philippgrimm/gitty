...
test('it parses porcelain v2 status with clean working tree', function () {
    Process::fake([
        'git status --porcelain=v2 --branch -uall' => GitOutputFixtures::statusClean(),
    ]);
...
test('it parses porcelain v2 status with unstaged changes', function () {
    Process::fake([
        'git status --porcelain=v2 --branch -uall' => GitOutputFixtures::statusWithUnstagedChanges(),
    ]);
...
test('it parses porcelain v2 status with staged changes', function () {
    Process::fake([
        'git status --porcelain=v2 --branch -uall' => GitOutputFixtures::statusWithStagedChanges(),
    ]);
...
test('it parses porcelain v2 status with renamed files', function () {
    Process::fake([
        'git status --porcelain=v2 --branch -uall' => GitOutputFixtures::statusWithRenamedFiles(),
    ]);
...
test('it detects detached HEAD state', function () {
    Process::fake([
        'git status --porcelain=v2 --branch -uall' => GitOutputFixtures::statusDetachedHead(),
    ]);
...
test('it calculates ahead/behind commits', function () {
    Process::fake([
        'git status --porcelain=v2 --branch -uall' => GitOutputFixtures::statusAheadBehind(),
    ]);
