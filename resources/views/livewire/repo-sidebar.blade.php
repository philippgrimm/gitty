    ...
    <div class="flex-1 overflow-y-auto">
        <div class="border-b-2 border-zinc-800">
            ...
            >
                ...
                <div class="text-zinc-500 transition-transform" :class="{ 'rotate-90': branchesOpen }">▶</div>
            </button>
                    ...
                    >
                        <div class="flex items-center gap-3 flex-1 min-w-0">
                            ...
                            <flux:tooltip :content="$branch['name']">
                                <div class="text-sm truncate {{ $branch['isCurrent'] ? 'text-green-400 font-bold' : 'text-zinc-200' }} group-hover:text-white transition-colors">
                                    {{ $branch['name'] }}
                                </div>
                            </flux:tooltip>
                        </div>
                        @if($branch['lastCommitSha'])
                            <div class="text-xs text-zinc-600 font-mono">{{ substr($branch['lastCommitSha'], 0, 7) }}</div>
                        @endif
                    </button>
                @empty
                    <div class="px-4 py-3 text-xs text-zinc-600 uppercase tracking-wider">No branches</div>
                @endforelse
            </div>
        </div>
        ...
        <div class="border-b-2 border-zinc-800">
            ...
            >
                ...
                <div class="text-zinc-500 transition-transform" :class="{ 'rotate-90': remotesOpen }">▶</div>
            </button>
                ...
                @forelse($remotes as $remote)
                    <div class="px-4 py-2.5">
                        ...
                        <flux:tooltip :content="$remote['fetchUrl']">
                            <div class="text-xs text-zinc-500 truncate">{{ $remote['fetchUrl'] }}</div>
                        </flux:tooltip>
                    </div>
                @empty
                    <div class="px-4 py-3 text-xs text-zinc-600 uppercase tracking-wider">No remotes</div>
                @endforelse
            </div>
        </div>
        ...
        <div class="border-b-2 border-zinc-800">
            ...
            >
                ...
                <div class="text-zinc-500 transition-transform" :class="{ 'rotate-90': tagsOpen }">▶</div>
            </button>
            <div x-show="tagsOpen" x-collapse class="divide-y divide-zinc-800">
                @forelse($tags as $tag)
                    <div class="px-4 py-2.5 hover:bg-zinc-900 transition-colors flex items-center justify-between gap-3">
                        ...
                        @if($tag['sha'])
                            <div class="text-xs text-zinc-600 font-mono">{{ substr($tag['sha'], 0, 7) }}</div>
                        @endif
                    </div>
                @empty
                    <div class="px-4 py-3 text-xs text-zinc-600 uppercase tracking-wider">No tags</div>
                @endforelse
            </div>
        </div>
        ...
        <div class="border-b-2 border-zinc-800">
            ...
            >
                ...
                <div class="text-zinc-500 transition-transform" :class="{ 'rotate-90': stashesOpen }">▶</div>
            </button>
            <div x-show="stashesOpen" x-collapse class="divide-y divide-zinc-800">
                @forelse($stashes as $stash)
                    <div class="px-4 py-2.5 hover:bg-zinc-900 transition-colors">
                        ...
                        @if($stash['sha'])
                            <div class="text-xs text-zinc-600 font-mono mt-1">{{ substr($stash['sha'], 0, 7) }}</div>
                        @endif
                    </div>
                @empty
                    <div class="px-4 py-3 text-xs text-zinc-600 uppercase tracking-wider">No stashes</div>
                @endforelse
            </div>
        </div>
    </div>
